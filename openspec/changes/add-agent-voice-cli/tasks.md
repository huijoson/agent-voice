...
- [ ] 3.1 RED/GREEN `DEFAULT_CONFIG` matches the spec exactly; test asserts shape
      and non-empty messages
- [ ] 3.2 RED/GREEN `ensureConfigDir()` / `saveConfig()` write pretty JSON into a
      temp dir; round-trip test
- [ ] 3.3 RED/GREEN `loadConfig()`: returns parsed config; throws "run init" when
      missing; throws parse error on invalid JSON (temp-dir tests)
- [ ] 3.4 RED/GREEN `initConfig({ force })`: creates dir+file; protects existing;
      overwrites when force (temp-dir tests)

## 4. Speakers (test-first, mocked spawn)

- [ ] 4.1 RED/GREEN `speaker/macos.ts`: builds `say` arg array; adds `-v <voice>`;
      passes text literally; resolves on exit 0, rejects on non-zero (mock runner)
- [ ] 4.2 RED/GREEN `speaker/windows.ts`: builds PowerShell `SpeechSynthesizer`
...
- [ ] 4.3 RED/GREEN `speaker/unsupported.ts`: rejects with clear message, spawns
      nothing
- [ ] 4.4 RED/GREEN `speaker/index.ts`: `getSpeaker(platform)` selection mapping

## 5. CLI commands (integration tests)

- [ ] 5.1 RED/GREEN command handlers for `init`, `speak --event`, `say`,
      `install --target` (pure functions taking injected config/speaker/io)
- [ ] 5.2 RED/GREEN `speak` maps event→message, errors on unknown event (exit !=0),
      guides to init when config missing
- [ ] 5.3 RED/GREEN `say` speaks literal text, errors on empty input
- [ ] 5.4 `cli.ts`: wire commander; `index.ts`: shebang + `process.argv` entry +
      top-level error handling → non-zero exit
- [ ] 5.5 Integration test: `init` then `speak --event done` core flow with a
      mocked speaker and temp home dir

## 6. Claude hook installer (test-first, fixtures)

- [ ] 6.1 RED/GREEN creates `settings.json` when absent (temp home)
- [ ] 6.2 RED/GREEN timestamped backup `settings.json.bak-YYYYMMDDHHmmss` is made
      from existing settings before write
- [ ] 6.3 RED/GREEN non-destructive merge: fixture with existing keys+hooks keeps
      everything and adds Stop→done and Notification→needInput
- [ ] 6.4 RED/GREEN idempotency: second run does not duplicate entries

## 7. Codex hook stub (test-first)

- [ ] 7.1 RED/GREEN `hooks/codex.ts` `installCodexHook()` prints
      "Codex hook install is not fully implemented yet", returns without throwing,
      TODOs documented

## 8. Documentation

- [ ] 8.1 Write `README.md`: purpose, install, build, local dev, init, test speech,
...
- [ ] 9.1 `npm run typecheck`, `npm run build`, `npm test`, `npm run coverage` all
